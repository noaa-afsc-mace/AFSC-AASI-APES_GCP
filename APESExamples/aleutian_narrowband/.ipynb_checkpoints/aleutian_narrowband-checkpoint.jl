...
plot!(distances, depths, b, yflip=true, aspect_ratio=0.03, xlims=(0, 18), ylims=(5, 395),show = true)
...
d1 = plot(sv_plots..., layout=(5, 1), size=(800, 900), dpi=300, leftmargin=20px)
...
d2 = plot(p1, p2, size=(1000, 350), dpi=500, margin=15px)
...
d4 = heatmap(fmissing.(solution_mcmc, chn -> mean(chn[:ϵ])), yflip=true, c=:viridis)
savefig(joinpath(@__DIR__, "plots/epsilon.png"))
display(d4)
d5 = heatmap(fmissing.(solution_mcmc, chn -> mean(chn[:lp])), yflip=true, c=:viridis)
...
d6 = heatmap(rhats, yflip=true, clims=(0, 0.1), c=:viridis)
